...
from mainsite.utils import get_slice_and_page_end, redirect_with_data
...
def create(request, data, **kwargs):
    if request.method == 'POST':
        ...
        payer = Staff.objects.get(user=request.user)
        company = Company.objects.get(unique_id=company_uuid)
        new_salary = Salary(payer_id=payer, payee=payee, company=company, base_salary=base_salary, bonus=bonus, total=total, date=date)
        new_salary.save()
